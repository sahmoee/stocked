...
- Sections 45–46 add 24 device checks. The checkbook now contains **374 checks**, including 104 appended
  recipe/browser/QA/shared-cooking checks; none were automatically marked as a physical-device pass.

## Build automation scope

Stocked, Atlas, Nova (iOS and tvOS), The SESH and ReelPromo-iOS now use shared-scheme pre-actions and
per-target plist stamping. The generated plist is not declared as an output, avoiding an Xcode
dependency-graph cycle; user-script sandboxing is disabled only for this repo-owned local phase.
Mac-only projects remain excluded. Stocked owns the identical
...
The backward-compatible Worker patch is deployed. No migration or legacy ownership guessing is
required; existing optional-field behavior remains the fallback for older clients. The same
production rollout also exposes additive household-cooking coordination and maintenance health.
...
| Worker test suite, including old-client identity preservation | 87 passed |
| Worker type-check, deployment and production health/version smoke | Passed; deployed 2026-09-03 |
...
| Stocked TestFlight release | Build 120 uploaded; marketing version remained 5 |
| Simulator builds/tests and direct device installs | Not performed |
