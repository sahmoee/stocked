...
@main struct RecipeFinderCoreChecks {
    static func main() {
        ...
        check(FinderChoice.finderCuisines(for: "Jamaican") == [.jamaican, .caribbean], "Jamaican routes to specific and parent tiles")
        check(FinderChoice.finderCuisines(for: "Tex-Mex") == [.mexican, .american], "Tex-Mex routes to relevant visible tiles")
        check(FinderChoice.finderCuisines(for: "Brazilian") == [.latin], "Brazilian routes to Latin")
        check(FinderChoice.finderCuisines(for: "Turkish") == [.middleEastern, .mediterranean], "Turkish routes to regional tiles")
        check(FinderChoice.finderCuisines(for: "Other").isEmpty, "Unclassified recipes do not leak into cuisine tiles")
        var flow = FinderFlow()
        ...
        check(request.count == 2, "Partial preview publishes loaded count")
        check(request.canReportCount(catalogueLoading: false, query: "soup"), "Typing reports loaded matches")
        check(request.preview(first, count: 200), "Later partial count is accepted")
        check(request.count == 200, "Growing scan advances loaded count")
        ...
        check(request.canReportCount(catalogueLoading: false, query: ""), "Completed load reports total")
        check(request.canReportCount(catalogueLoading: true, query: ""), "Completed query reports total during background download")
        check(request.canReportCount(catalogueLoading: true, query: " \n"), "Background download does not hide completed total")
        check(request.canReportCount(catalogueLoading: true, query: "soup"), "Completed typed search can report cached matches")
        check(!request.preview(second, count: 999), "Completed count cannot be overwritten by a preview")
        let third = request.begin(); request.cancel()
        check(!request.complete(third, count: 999), "Cancelled completion discarded")
        check(!request.canReportCount(catalogueLoading: false, query: "soup"), "Cancelled search never reports zero as final")
        let fourth = request.begin()
        check(request.fail(fourth) && request.phase == .failed, "Error state")
        check(!request.canReportCount(catalogueLoading: false, query: ""), "Failure never masquerades as a finished count")
        let retry = request.begin()
        check(request.complete(retry, count: 0) && request.count == 0 && request.phase == .ready, "Retry empty results")
        check(request.canReportCount(catalogueLoading: false, query: ""), "Genuine final zero is reportable")
