...
// braise, roast). "Your lamb is cooking — 34 minutes hands-off. What would you
// like to do?" The whole point is that DOING NOTHING is a first-class, framed
// choice, and the other options fit the remaining time and free appliances.
//
// Options:
//   • Nothing — the entrée is enough; rest or leave the kitchen. (Success.)
//   • Add a side that fits the remaining window (5-min vs 15-min buckets),
//     filtered to inventory + the appliances NOT occupied by the main cook.
//   • Use another appliance — same idea, framed around the free device.
//   • Prep for another meal — overlapping-ingredient prep (hands to Batch 10).
//   • Clean as you go — a short cleanup sequence for items no longer needed.
//
// Side ideas respect inventory, expiry, remaining time, effort level, and the
// equipment still free while the main appliance is busy. Chosen sides are
// recorded on the session. This screen is opened from the cooking flow with the
// method's total time; it estimates the remaining hands-off window.
// ─────────────────────────────────────────────────────────────────
...
struct HandsOffOpportunityView: View {
        ...
        VStack(spacing: 10) {
            ...
            if effort.allowsCompounding {
                optionCard(icon: "square.stack.3d.up", title: "Prep for another meal",
                    ...
                    // Compounding prep detail arrives in the next batch; for now
                    // this routes nowhere destructive and simply closes.
    ...
    // MARK: Side suggestions

    /// Quick, low-effort side ideas that fit the window and current inventory.
    /// Honest and simple: a curated list intersected with what's in stock.
    private func sideCandidates(maxMinutes: Int) -> [String] {
        let quick = ["Bagged salad", "Sautéed spinach", "Microwave rice", "Sliced cucumber", "Steamed broccoli", "Buttered peas"]
        ...
        // Prefer ideas whose key ingredient is in stock; keep a couple regardless.
