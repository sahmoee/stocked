...
final class ShareViewController: UIViewController {
    ...
    private func persistAndOpen(url: String?, text: String?, image: Data?) {
        // NSLog so it shows in Console.app even though extensions can't use the app's Log helper.
        NSLog("StockedShareExt: persist url=\(url ?? "nil") textLen=\(text?.count ?? 0) hasImage=\(image != nil)")
        guard let defaults = UserDefaults(suiteName: appGroupID) else {
            NSLog("StockedShareExt: UserDefaults(suiteName: \(appGroupID)) is NIL — App Group not enabled on the EXTENSION target")
            complete(); return
        }
        ...
        NSLog("StockedShareExt: wrote payload, keys=\(payload.keys.joined(separator: ","))")
